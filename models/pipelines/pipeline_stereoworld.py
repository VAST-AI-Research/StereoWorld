...
# from ...models.wan_vae import AutoencoderKLWan3_8 as AutoencoderKLWan
from diffusers.models import AutoencoderKLWan
from transformers import AutoTokenizer
from transformers import UMT5EncoderModel as WanT5EncoderModel
...
class StereoWorldPipeline(DiffusionPipeline):
    r"""
    ...
    def __call__(
        ...
        raymap: Optional[torch.FloatTensor] = None,
        num_frames: int = 49,
        ...
        latent_channels = getattr(self.vae.config, 'z_dim', None) or self.vae.config.latent_channels
        ...
        target_shape = (latent_channels, (num_frames - 1) // self.vae.temporal_compression_ratio + 1, height // self.vae.spatial_compression_ratio, width // self.vae.spatial_compression_ratio)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if raymap is not None:
                    raymap_input = raymap.unsqueeze(0) if raymap.dim() == 4 else raymap
                    raymap_input = torch.cat([raymap_input] * 2) if do_classifier_free_guidance else raymap_input
                    latent_model_input = torch.cat([latent_model_input, raymap_input.to(device, weight_dtype)], dim=1)
                ...
                if control_camera_video is not None and isinstance(control_camera_video, torch.Tensor):
                    ...
                    for key, value in control_camera_latents.items():
                        v = torch.cat([value] * 2) if do_classifier_free_guidance else value
                        if v.is_floating_point():
                            v = v.to(device, weight_dtype)
                        else:
                            v = v.to(device)
                        control_camera_latents_input[key] = v
