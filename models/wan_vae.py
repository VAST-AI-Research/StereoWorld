...
class AutoencoderKLWan3_8(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def from_pretrained(cls, pretrained_model_path, additional_kwargs={}):
        ...
        # Support directory path: auto-detect weight file inside
        if os.path.isdir(pretrained_model_path):
            safetensors_path = os.path.join(pretrained_model_path, "diffusion_pytorch_model.safetensors")
            bin_path = os.path.join(pretrained_model_path, "diffusion_pytorch_model.bin")
            if os.path.exists(safetensors_path):
                pretrained_model_path = safetensors_path
            elif os.path.exists(bin_path):
                pretrained_model_path = bin_path
            else:
                raise FileNotFoundError(f"No weight file found in {pretrained_model_path}")
        ...
        # Add "model." prefix only if keys don't already have it
